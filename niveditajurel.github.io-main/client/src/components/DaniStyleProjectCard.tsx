...
export function DaniStyleProjectCard({
    ...
    const CardContent = () => (
        <div
            className={`group relative overflow-hidden rounded-2xl shadow-xl/20 hover:shadow-2xl transition-all duration-500 cursor-pointer ${isNomadAI
        ...
        >
            <div className={`w-full h-64 flex items-center justify-center ${isNomadAI ? 'bg-black p-4' : ''
                }`}>
                <img
                    src={slides[0]?.img}
                    alt={project.title}
                    className={`rounded-2xl transition-all duration-500 ${isNomadAI
            ...
            <div className="absolute bottom-0 left-0 w-full p-4 flex flex-col gap-2 z-10">
                <h3 className={`text-lg font-semibold mb-1 drop-shadow-lg ${isSkingenius ? 'text-black' : 'text-white'
                    }`}>{project.title}</h3>
                <div className="flex flex-wrap gap-2">
                    {project.tags.split(', ').map((rawTag) => {
                        ...
                        return (
                            <span
                                key={tag}
                                className={`px-3 py-1 text-xs font-medium rounded-full border ${isSkingenius
            ...
            >
                <div className="flex flex-col items-center justify-center h-full px-8 text-center">
                    <h3 className={`text-2xl font-bold mb-2 ${isNomadAI ? 'text-white' : 'text-ink dark:text-base'
                        }`}>{project.title}</h3>
                    <div className="flex flex-wrap gap-2 mb-4 justify-center">
                        {project.tags.split(', ').map((rawTag) => {
                            ...
                            return (
                                <span
                                    key={tag}
                                    className={`px-3 py-1 text-xs font-medium rounded-full border ${isNomadAI
                    ...
                    <p className={`text-lg max-w-md ${isNomadAI ? 'text-white/80' : 'text-ink/80 dark:text-base/80'
                        }`}>
                        {slides[0]?.blurb}
                    </p>
